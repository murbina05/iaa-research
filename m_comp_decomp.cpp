...
auto main(int argc, char** argv) -> int {
    ...
    uint32_t size = std::filesystem::file_size(table_path);
    // std::vector<uint8_t> buffer(size);

    const std::unique_ptr<uint8_t[]> unique_buffer = std::make_unique<uint8_t[]>(size);
    ...
    status = qpl_huffman_table_deserialize(buffer, size,DEFAULT_ALLOCATOR_C, &huffman_table);
    ...
    for (const auto& path : std::filesystem::directory_iterator(dataset_path)) {
        ...
        std::ofstream outfile("large_huffman_table_H.bin", std::ios::binary);
