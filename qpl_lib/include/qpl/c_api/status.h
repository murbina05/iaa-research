...
// <--- Common
     ...
     * \deprecated Use QPL_STS_TRANSFER_SIZE_INVALID for buffers exceeding max size errors
    */
    QPL_STS_BUFFER_TOO_LARGE_ERR QPL_DEPRECATED("QPL_STS_BUFFER_TOO_LARGE_ERR is deprecated, use QPL_STS_TRANSFER_SIZE_INVALID for buffers exceeding max size errors") = QPL_PARAMETER_ERROR(8U),
...
/* --- Corresponds to the status codes in the Completion Record from Intel® In-Memory Analytics Accelerator --- */
     ...
     * can be returned by Intel QPL on submission if no available workqueues support transfer size
     **/
    QPL_STS_TRANSFER_SIZE_INVALID       = QPL_OPERATION_STATUS(19U),
